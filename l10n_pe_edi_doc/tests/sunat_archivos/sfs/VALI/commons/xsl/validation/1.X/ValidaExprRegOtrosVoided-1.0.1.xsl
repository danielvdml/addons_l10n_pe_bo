  ...
  <xsl:template match="/*">
    ...
    <xsl:choose>
      <xsl:when test="not(string(./cac:AccountingSupplierParty/cbc:CustomerAssignedAccountID))">
        <xsl:call-template name="rejectCall">
          ...
          <xsl:with-param name="errorMessage" select="'2217 Error resumen diario de reversiones'"/>
        </xsl:call-template>
        <xsl:call-template name="rejectCall">
          <xsl:with-param name="errorCode" select="2217"/>
          <xsl:with-param name="errorMessage" select="'Error resumen diario de reversiones'"/>
        </xsl:call-template>
      </xsl:when>
      <xsl:otherwise>
        <xsl:if test="not(regexp:match(./cac:AccountingSupplierParty/cbc:CustomerAssignedAccountID,&quot;^[0-9]{11}$&quot;))">
          <xsl:call-template name="rejectCall">
            ...
            <xsl:with-param name="errorMessage" select="'2216 Error resumen diario de reversiones'"/>
          </xsl:call-template>
        </xsl:if>
      </xsl:otherwise>
    </xsl:choose>
    ...
    <xsl:choose>
      <xsl:when test="not(string(./cbc:ID))">
        <xsl:call-template name="rejectCall">
          ...
          <xsl:with-param name="errorMessage" select="'2284 Error resumen diario de reversiones'"/>
        </xsl:call-template>
      </xsl:when>
      <xsl:otherwise>
        <xsl:if test="not(regexp:match(./cbc:ID,&quot;[R][R]-[0-9]{8}-[0-9]{1,5}&quot;))">
          <xsl:call-template name="rejectCall">
            <xsl:with-param name="errorCode" select="2673"/>
            <xsl:with-param name="errorMessage" select="'2673 Error resumen diario de reversiones'"/>
          </xsl:call-template>
        </xsl:if>
      </xsl:otherwise>
    </xsl:choose>
    ...
    <xsl:choose>
      <xsl:when test="not(string(./cbc:CustomizationID))">
        <xsl:call-template name="rejectCall">
          ...
          <xsl:with-param name="errorMessage" select="'2073 Error resumen diario de reversiones'"/>
        </xsl:call-template>
      </xsl:when>
      <xsl:otherwise>
        <xsl:if test="not(./cbc:CustomizationID=&quot;1.0&quot;)">
          <xsl:call-template name="rejectCall">
            ...
            <xsl:with-param name="errorMessage" select="'2072 Error resumen diario de reversiones'"/>
          </xsl:call-template>
        </xsl:if>
      </xsl:otherwise>
    </xsl:choose>
    ...
    <xsl:choose>
      <xsl:when test="not(string(./cbc:UBLVersionID))">
        <xsl:call-template name="rejectCall">
          ...
          <xsl:with-param name="errorMessage" select="'2075 Error resumen diario de reversiones'"/>
        </xsl:call-template>
      </xsl:when>
      <xsl:otherwise>
        <xsl:if test="not(./cbc:UBLVersionID=&quot;2.0&quot;)">
          <xsl:call-template name="rejectCall">
            ...
            <xsl:with-param name="errorMessage" select="'2074 Error resumen diario de reversiones'"/>
          </xsl:call-template>
        </xsl:if>
      </xsl:otherwise>
    </xsl:choose>
    <!-- 6.- Tipo de Documento del Emisor - RUC -->
    ...
    <xsl:if test="not(string(./cac:AccountingSupplierParty/cbc:AdditionalAccountID))">
      <xsl:call-template name="rejectCall">
        ...
        <xsl:with-param name="errorMessage" select="'2288 Error resumen diario de reversiones'"/>
      </xsl:call-template>
    </xsl:if>
    <xsl:if test="not(regexp:match(./cac:AccountingSupplierParty/cbc:AdditionalAccountID,&quot;^[6]{1}$&quot;))">
      <xsl:call-template name="rejectCall">
        ...
        <xsl:with-param name="errorMessage" select="'2287 Error resumen diario de reversiones'"/>
      </xsl:call-template>
    </xsl:if>
    <!-- 7.- Apellidos y nombres o denominacion o razon social Emisor -->
    ...
    <xsl:choose>
      <xsl:when test="not(string(./cac:AccountingSupplierParty/cac:Party/cac:PartyLegalEntity/cbc:RegistrationName))">
        <xsl:call-template name="rejectCall">
          ...
          <xsl:with-param name="errorMessage" select="'2229 Error resumen diario de reversiones'"/>
        </xsl:call-template>
      </xsl:when>
      <xsl:otherwise>
    ...
    <!-- 8.- Fecha de emision del documento -->
    <!-- <xsl:value-of select="./cbc:ReferenceDate"/> -->
    <xsl:choose>
      <xsl:when test="(not(./cbc:ReferenceDate))">
        <xsl:call-template name="rejectCall">
          <xsl:with-param name="errorCode" select="2303"/>
          <xsl:with-param name="errorMessage" select="'2303 Error resumen diario de reversiones'"/>
        </xsl:call-template>
      </xsl:when>
      <xsl:otherwise>
        <xsl:if test="not(regexp:match(./cbc:ReferenceDate,&quot;^[0-9]{4}-[0-9]{2}-[0-9]{2}$&quot;))">
          <xsl:call-template name="rejectCall">
            ...
            <xsl:with-param name="errorMessage" select="'2302 Error resumen diario de reversiones'"/>
          </xsl:call-template>
        </xsl:if>
      </xsl:otherwise>
    </xsl:choose>
    ...
    <xsl:if test="not(regexp:match($fechaEmisionDDMMYYYY,&quot;^(?:(?:0?[1-9]|1\d|2[0-8])(\/|-)(?:0?[1-9]|1[0-2]))(\/|-)(?:[1-9]\d\d\d|\d[1-9]\d\d|\d\d[1-9]\d|\d\d\d[1-9])$|^(?:(?:31(\/|-)(?:0?[13578]|1[02]))|(?:(?:29|30)(\/|-)(?:0?[1,3-9]|1[0-2])))(\/|-)(?:[1-9]\d\d\d|\d[1-9]\d\d|\d\d[1-9]\d|\d\d\d[1-9])$|^(29(\/|-)0?2)(\/|-)(?:(?:0[48]00|[13579][26]00|[2468][048]00)|(?:\d\d)?(?:0[48]|[2468][048]|[13579][26]))$&quot;))">
      <xsl:call-template name="rejectCall">
        ...
        <xsl:with-param name="errorMessage" select="'2304 Error resumen diario de reversiones'"/>
      </xsl:call-template>
    </xsl:if>
    ...
    <xsl:choose>
      <xsl:when test="(not(./cbc:IssueDate))">
        <xsl:call-template name="rejectCall">
          ...
          <xsl:with-param name="errorMessage" select="'2299 Error resumen diario de reversiones'"/>
        </xsl:call-template>
      </xsl:when>
      <xsl:otherwise>
        <xsl:if test="not(regexp:match(./cbc:IssueDate,&quot;^[0-9]{4}-[0-9]{2}-[0-9]{2}$&quot;))">
          <xsl:call-template name="rejectCall">
            ...
            <xsl:with-param name="errorMessage" select="'2298 Error resumen diario de reversiones'"/>
          </xsl:call-template>
        </xsl:if>
      </xsl:otherwise>
    </xsl:choose>
    ...
    <xsl:if test="not(regexp:match($fechaEmisionComDDMMYYYY,&quot;^(?:(?:0?[1-9]|1\d|2[0-8])(\/|-)(?:0?[1-9]|1[0-2]))(\/|-)(?:[1-9]\d\d\d|\d[1-9]\d\d|\d\d[1-9]\d|\d\d\d[1-9])$|^(?:(?:31(\/|-)(?:0?[13578]|1[02]))|(?:(?:29|30)(\/|-)(?:0?[1,3-9]|1[0-2])))(\/|-)(?:[1-9]\d\d\d|\d[1-9]\d\d|\d\d[1-9]\d|\d\d\d[1-9])$|^(29(\/|-)0?2)(\/|-)(?:(?:0[48]00|[13579][26]00|[2468][048]00)|(?:\d\d)?(?:0[48]|[2468][048]|[13579][26]))$&quot;))">
      <xsl:call-template name="rejectCall">
        ...
        <xsl:with-param name="errorMessage" select="'2300 Error resumen diario de reversiones'"/>
      </xsl:call-template>
    </xsl:if>
    <!-- *** SFS 2.0 INICIO - seconds() -->
    <!-- *** xsl:variable name="issuedate" select="./cbc:IssueDate"/>
    ...
    <xsl:choose>
      <xsl:when test="not((cac:Signature/cbc:ID))">
        <xsl:call-template name="rejectCall">
          ...
          <xsl:with-param name="errorMessage" select="'2076 Error resumen diario de reversiones'"/>
        </xsl:call-template>
      </xsl:when>
      <xsl:otherwise>
        <xsl:if test="not(regexp:match(cac:Signature/cbc:ID,&quot;^(?!\s*$).{1,3000}&quot;))">
          <xsl:call-template name="rejectCall">
            ...
            <xsl:with-param name="errorMessage" select="'2077 Error resumen diario de reversiones'"/>
          </xsl:call-template>
        </xsl:if>
      </xsl:otherwise>
    </xsl:choose>
    <xsl:if test="not(cac:Signature/cac:SignatoryParty/cac:PartyIdentification/cbc:ID)">
      <xsl:call-template name="rejectCall">
        ...
        <xsl:with-param name="errorMessage" select="'2079 Error resumen diario de reversiones'"/>
      </xsl:call-template>
    </xsl:if>
    ...
    <xsl:choose>
      <xsl:when test="not(cac:Signature/cac:SignatoryParty/cac:PartyName/cbc:Name)">
        <xsl:call-template name="rejectCall">
          ...
          <xsl:with-param name="errorMessage" select="'2081 Error resumen diario de reversiones'"/>
        </xsl:call-template>
      </xsl:when>
      <xsl:otherwise>
        <xsl:if test="not(regexp:match(cac:Signature/cac:SignatoryParty/cac:PartyName/cbc:Name,&quot;^[^\s].{1,100}&quot;))">
          <xsl:call-template name="rejectCall">
            ...
            <xsl:with-param name="errorMessage" select="'2080 Error resumen diario de reversiones'"/>
          </xsl:call-template>
        </xsl:if>
      </xsl:otherwise>
    </xsl:choose>
    <xsl:if test="not(cac:Signature/cac:DigitalSignatureAttachment/cac:ExternalReference/cbc:URI)">
      <xsl:call-template name="rejectCall">
        ...
        <xsl:with-param name="errorMessage" select="'2083 Error resumen diario de reversiones'"/>
      </xsl:call-template>
    </xsl:if>
    <xsl:choose>
      ...
      <xsl:otherwise>
        <xsl:if test="not(regexp:match(ext:UBLExtensions/ext:UBLExtension/ext:ExtensionContent/ds:Signature/@Id,&quot;^[^\s].{1,100}&quot;))">
          <xsl:call-template name="rejectCall">
            ...
            <xsl:with-param name="errorMessage" select="'2084 Error resumen diario de reversiones'"/>
          </xsl:call-template>
        </xsl:if>
      </xsl:otherwise>
    </xsl:choose>
    <xsl:choose>
      <xsl:when test="not(ext:UBLExtensions/ext:UBLExtension/ext:ExtensionContent/ds:Signature/ds:SignedInfo/ds:CanonicalizationMethod/@Algorithm)">
        <xsl:call-template name="rejectCall">
          ...
          <xsl:with-param name="errorMessage" select="'2087 Error resumen diario de reversiones'"/>
        </xsl:call-template>
      </xsl:when>
      <xsl:otherwise>
        <xsl:if test="not(regexp:match(ext:UBLExtensions/ext:UBLExtension/ext:ExtensionContent/ds:Signature/ds:SignedInfo/ds:CanonicalizationMethod/@Algorithm,&quot;^[^\s].{1,100}&quot;))">
          <xsl:call-template name="rejectCall">
            ...
            <xsl:with-param name="errorMessage" select="'2086 Error resumen diario de reversiones'"/>
          </xsl:call-template>
        </xsl:if>
      </xsl:otherwise>
    </xsl:choose>
    <xsl:choose>
      <xsl:when test="not(ext:UBLExtensions/ext:UBLExtension/ext:ExtensionContent/ds:Signature/ds:SignedInfo/ds:SignatureMethod/@Algorithm)">
        <xsl:call-template name="rejectCall">
          ...
          <xsl:with-param name="errorMessage" select="'2089 Error resumen diario de reversiones'"/>
        </xsl:call-template>
      </xsl:when>
      <xsl:otherwise>
        <xsl:if test="not(regexp:match(ext:UBLExtensions/ext:UBLExtension/ext:ExtensionContent/ds:Signature/ds:SignedInfo/ds:SignatureMethod/@Algorithm,&quot;^[^\s].{1,100}&quot;))">
          <xsl:call-template name="rejectCall">
            ...
            <xsl:with-param name="errorMessage" select="'2088 Error resumen diario de reversiones'"/>
          </xsl:call-template>
        </xsl:if>
      </xsl:otherwise>
    </xsl:choose>
    <xsl:choose>
      <xsl:when test="not(ext:UBLExtensions/ext:UBLExtension/ext:ExtensionContent/ds:Signature/ds:SignedInfo/ds:Reference/@URI)">
        <xsl:call-template name="rejectCall">
          ...
          <xsl:with-param name="errorMessage" select="'2091 Error resumen diario de reversiones'"/>
        </xsl:call-template>
      </xsl:when>
      <xsl:otherwise>
        <xsl:if test="string(ext:UBLExtensions/ext:UBLExtension/ext:ExtensionContent/ds:Signature/ds:SignedInfo/ds:Reference/@URI)">
          <xsl:call-template name="rejectCall">
            ...
            <xsl:with-param name="errorMessage" select="'2090 Error resumen diario de reversiones'"/>
          </xsl:call-template>
        </xsl:if>
      </xsl:otherwise>
    </xsl:choose>
    <xsl:choose>
      <xsl:when test="not(ext:UBLExtensions/ext:UBLExtension/ext:ExtensionContent/ds:Signature/ds:SignedInfo/ds:Reference/ds:Transforms/ds:Transform/@Algorithm)">
        <xsl:call-template name="rejectCall">
          ...
          <xsl:with-param name="errorMessage" select="'2093 Error resumen diario de reversiones'"/>
        </xsl:call-template>
      </xsl:when>
      <xsl:otherwise>
        <xsl:if test="not(regexp:match(ext:UBLExtensions/ext:UBLExtension/ext:ExtensionContent/ds:Signature/ds:SignedInfo/ds:Reference/ds:Transforms/ds:Transform/@Algorithm,&quot;^[^\s].{1,100}&quot;))">
          <xsl:call-template name="rejectCall">
            ...
            <xsl:with-param name="errorMessage" select="'2092 Error resumen diario de reversiones'"/>
          </xsl:call-template>
        </xsl:if>
      </xsl:otherwise>
    </xsl:choose>
    <xsl:choose>
      <xsl:when test="not(ext:UBLExtensions/ext:UBLExtension/ext:ExtensionContent/ds:Signature/ds:SignedInfo/ds:Reference/ds:DigestMethod/@Algorithm)">
        <xsl:call-template name="rejectCall">
          ...
          <xsl:with-param name="errorMessage" select="'2095 Error resumen diario de reversiones'"/>
        </xsl:call-template>
      </xsl:when>
      <xsl:otherwise>
        <xsl:if test="not(regexp:match(ext:UBLExtensions/ext:UBLExtension/ext:ExtensionContent/ds:Signature/ds:SignedInfo/ds:Reference/ds:DigestMethod/@Algorithm,&quot;^[^\s].{1,100}&quot;))">
          <xsl:call-template name="rejectCall">
            ...
            <xsl:with-param name="errorMessage" select="'2094 Error resumen diario de reversiones'"/>
          </xsl:call-template>
        </xsl:if>
      </xsl:otherwise>
    </xsl:choose>
    <xsl:choose>
      <xsl:when test="not(ext:UBLExtensions/ext:UBLExtension/ext:ExtensionContent/ds:Signature/ds:SignedInfo/ds:Reference/ds:DigestValue)">
        <xsl:call-template name="rejectCall">
          ...
          <xsl:with-param name="errorMessage" select="'2097 Error resumen diario de reversiones'"/>
        </xsl:call-template>
      </xsl:when>
      <xsl:otherwise>
        <!--
    ...
    <xsl:choose>
      <xsl:when test="not(ext:UBLExtensions/ext:UBLExtension/ext:ExtensionContent/ds:Signature/ds:SignatureValue)">
        <xsl:call-template name="rejectCall">
          ...
          <xsl:with-param name="errorMessage" select="'2099 Error resumen diario de reversiones'"/>
        </xsl:call-template>
      </xsl:when>
      <xsl:otherwise>
        <xsl:if test="not(regexp:match(ext:UBLExtensions/ext:UBLExtension/ext:ExtensionContent/ds:Signature/ds:SignatureValue,&quot;[A-Za-z0-9+/=\s]{100,}&quot;))">
          <xsl:call-template name="rejectCall">
            ...
            <xsl:with-param name="errorMessage" select="'2098 Error resumen diario de reversiones'"/>
          </xsl:call-template>
        </xsl:if>
      </xsl:otherwise>
    </xsl:choose>
    <xsl:choose>
      <xsl:when test="not(ext:UBLExtensions/ext:UBLExtension/ext:ExtensionContent/ds:Signature/ds:KeyInfo/ds:X509Data/ds:X509Certificate)">
        <xsl:call-template name="rejectCall">
          ...
          <xsl:with-param name="errorMessage" select="'2101 Error resumen diario de reversiones'"/>
        </xsl:call-template>
      </xsl:when>
      <xsl:otherwise>
        <xsl:if test="not(regexp:match(ext:UBLExtensions/ext:UBLExtension/ext:ExtensionContent/ds:Signature/ds:KeyInfo/ds:X509Data/ds:X509Certificate,&quot;[A-Za-z0-9+/=\s]{100,}&quot;))">
          <xsl:call-template name="rejectCall">
            ...
            <xsl:with-param name="errorMessage" select="'2100 Error resumen diario de reversiones'"/>
          </xsl:call-template>
        </xsl:if>
      </xsl:otherwise>
    </xsl:choose>
    ...
    <xsl:for-each select="sac:VoidedDocumentsLine">
      ...
      <xsl:choose>
        <xsl:when test="not(string(cbc:LineID))">
          <xsl:call-template name="rejectCall">
            ...
            <xsl:with-param name="errorMessage" select="'2307 Error resumen diario de reversiones'"/>
          </xsl:call-template>
        </xsl:when>
        <xsl:when test="not(regexp:match(./cbc:LineID,'^[0-9]{1,5}?$'))">
          <xsl:call-template name="rejectCall">
            <xsl:with-param name="errorCode" select="2305"/>
            <xsl:with-param name="errorMessage" select="'2305 Error resumen diario de reversiones'"/>
          </xsl:call-template>
        </xsl:when>
        <!-- <xsl:when test="not(string(cbc:LineID))" >
        ...
        <xsl:when test="cbc:LineID &lt; 1">
          <xsl:call-template name="rejectCall">
            ...
            <xsl:with-param name="errorMessage" select="'2306 Error resumen diario de reversiones'"/>
          </xsl:call-template>
        </xsl:when>
        <xsl:otherwise/>
      </xsl:choose>
      ...
      <xsl:if test="count(key('by-document-line-id', cbc:LineID)) &gt; 1">
        <xsl:call-template name="rejectCall">
          ...
          <xsl:with-param name="errorMessage" select="concat('El numero de item esta duplicado: ', cbc:LineID)"/>
        </xsl:call-template>
      </xsl:if>
      <!-- Fin PaseYYY -->
      ...
      <xsl:choose>
        <xsl:when test="not(string(./cbc:DocumentTypeCode))">
          <xsl:call-template name="rejectCall">
            ...
            <xsl:with-param name="errorMessage" select="'2309 Error resumen diario de reversiones'"/>
          </xsl:call-template>
        </xsl:when>
        <xsl:otherwise>
          ...
          <xsl:if test="not(./cbc:DocumentTypeCode = '20' or ./cbc:DocumentTypeCode = '40')">
            <xsl:call-template name="rejectCall">
              ...
              <xsl:with-param name="errorMessage" select="'2308 Error resumen diario de reversiones'"/>
            </xsl:call-template>
          </xsl:if>
        </xsl:otherwise>
      </xsl:choose>
      <!-- 13.- Numero de serie de los documentos -->
      <!-- ./cbc:DocumentTypeCode = 20: RETENCION y 40: PERCEPCION -->
      <xsl:choose>
        <xsl:when test="not(string(./sac:DocumentSerialID))">
          <xsl:call-template name="rejectCall">
            <xsl:with-param name="errorCode" select="2311"/>
            <xsl:with-param name="errorMessage" select="'2311 Error resumen diario de reversiones'"/>
          </xsl:call-template>
        </xsl:when>
        <xsl:otherwise>
          ...
          <xsl:if test="./cbc:DocumentTypeCode='20' and not(regexp:match(./sac:DocumentSerialID,'(^[R][A-Z0-9]{3}|^[\d]{4})?$'))">
            <!-- Fin PAS20181U210300126 -->
            <xsl:call-template name="rejectCall">
              <xsl:with-param name="errorCode" select="2674"/>
              <xsl:with-param name="errorMessage" select="'2674 Error resumen diario de reversiones'"/>
            </xsl:call-template>
          </xsl:if>
          <!-- Ini PAS20181U210300126 -->
          <!-- <xsl:if test="./cbc:DocumentTypeCode='40' and not(regexp:match(./sac:DocumentSerialID,'^[P][A-Z0-9]{3}?$'))"> -->
          <xsl:if test="./cbc:DocumentTypeCode='40' and not(regexp:match(./sac:DocumentSerialID,'(^[P][A-Z0-9]{3}|^[\d]{4})?$'))">
            <!-- Fin PAS20181U210300126 -->
            <xsl:call-template name="rejectCall">
              <xsl:with-param name="errorCode" select="2675"/>
              <xsl:with-param name="errorMessage" select="'2675 Error resumen diario de reversiones'"/>
            </xsl:call-template>
          </xsl:if>
      ...
      <xsl:choose>
        <xsl:when test="not(string(./sac:DocumentNumberID))">
          <xsl:call-template name="rejectCall">
            ...
            <xsl:with-param name="errorMessage" select="'2313 Error resumen diario de reversiones'"/>
          </xsl:call-template>
        </xsl:when>
        <xsl:otherwise>
          <xsl:if test="not(regexp:match(./sac:DocumentNumberID,&quot;^[0-9]{1,8}?$&quot;))">
            <xsl:call-template name="rejectCall">
              ...
              <xsl:with-param name="errorMessage" select="'2312 Error resumen diario de reversiones'"/>
            </xsl:call-template>
          </xsl:if>
        </xsl:otherwise>
      </xsl:choose>
      ...
      <xsl:if test="count(key('by-document-id', concat(cbc:DocumentTypeCode, ' ', sac:DocumentSerialID, ' ', sac:DocumentNumberID))) &gt; 1">
        <xsl:call-template name="rejectCall">
          ...
          <xsl:with-param name="errorMessage" select="concat('El documento esta duplicado: ', cbc:DocumentTypeCode, '-', sac:DocumentSerialID, '-', sac:DocumentNumberID)"/>
        </xsl:call-template>
      </xsl:if>
      <!-- Fin PaseYYY -->
      <!--15.- Numero correlativo del documento de fin dentro de la serie-->
      <!--<xsl:value-of select="./sac:StartDocumentNumberID"/>-<xsl:value-of select="./sac:EndDocumentNumberID"/>-->
      <xsl:choose>
        <xsl:when test="not(string(./sac:VoidReasonDescription))">
          <xsl:call-template name="rejectCall">
            <xsl:with-param name="errorCode" select="2315"/>
            <xsl:with-param name="errorMessage" select="'2315 Error resumen diario de reversiones'"/>
          </xsl:call-template>
        </xsl:when>
        <xsl:otherwise>
          <xsl:if test="not(regexp:match(./sac:VoidReasonDescription,&quot;^(.{3,100})$&quot;))">
            <xsl:call-template name="rejectCall">
              ...
              <xsl:with-param name="errorMessage" select="'2314 Error resumen diario de reversiones'"/>
            </xsl:call-template>
          </xsl:if>
        </xsl:otherwise>
      </xsl:choose>
